...
void ShowEnergy() // вызов функции енергии
{
	infobox.Y = 3;
	SetConsoleCursorPosition(h, infobox);
	SetConsoleTextAttribute(h, Color::DARKYELLOW);
...
int main()
{
	...
	ShowHealth(); // function call
	 
	ShowEnergy(); // function call

	ShowSteps(); // function call
	...
	while (true)
	{
		if (_kbhit()) // если было нажатие на клавиши пользователем
		{
				...
				&& maze[position.Y][position.X - 1] != MazeObject::BORDER)
			{
				...
				if (energy <= 0)
				{
					int answer = MessageBoxA(0, "енергия закончилась!\n\nхотите начать сначала?", "ПОЛНЕЙШОЕ ПОРАЖЕНИЕ!", MB_YESNO);
					system("cls");
					if (answer == IDYES)
					{
						main();
